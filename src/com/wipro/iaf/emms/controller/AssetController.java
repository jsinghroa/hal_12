	...
	public String submit(
		...
		if (linkSelected.equals(Constants.LISTVIEW)) {
			...
			if (linkSelected.equals(Constants.ASSETCONFIG)) {
				...
				if (null != emmsDataForm.getSelectedRecordId()
					...
					try {
						...
						emmsDataForm.setAssetFormList(this.fetchAssetDetails(
								emmsDataForm.getSelectedRecordId(),
								emmsDataForm));
						for (int i = 0; i < emmsDataForm.getAssetFormList()
								.size(); i++) {
							if (emmsDataForm.getAssetFormList().get(i)
								...
								if (emmsDataForm.getAssetFormList().get(i)
										...
										&& emmsDataForm.getAssetFormList()
												...
												.equals("I")) {
									emmsDataForm
											.setInstallableFormList(installAssetController.fetchInstallableDetails(
													emmsDataForm
															.getSelectedRecordId(),
													emmsDataForm));
								}
				...
				try {
					if (null != configForm && configForm.size() > 0) {
						...
						for (AssetConfigForm assetConfigForm : configForm) {
							...
							assetConfigForm.setInstalledPNList(assetFormList
									.get(i).getInstalledPNList());
							assetConfigForm.setConditionCodes(assetFormList
							...
							if (assetConfigForm.getInstalledPN().length() <= 0
								...
								assetConfigForm
										.setErrorStatus(Constants.WARNING);
								emmsDataForm
								...
								String pnsn = assetConfigForm.getInstalledPN()
										+ ":"
										+ assetConfigForm.getInstalledSN();
								String pninlieu = assetConfigForm
										.getInstalledPN()
										+ ":"
										+ assetConfigForm.getInLieuPn();
								String sninlieu = assetConfigForm
										.getInstalledSN()
										+ ":"
										+ assetConfigForm.getInLieuPn();
								...
								assetConfigForm.setDateOfReciept(convertor
										.getDateTime(assetConfigForm
												.getDateOfReciept()));
								System.out.println(emmsDataForm
										.getSignalOutDate().length());

								
									if (!validate
											.contains(Constants.INLIEULENGTHERROR)
											&& !validate
													.contains(Constants.SNLENGTHERROR)) {
										System.out.println("INSIDE CONTAINS");
										halService.update(emmsDataForm,
												assetConfigForm);
									} else {
					...
					if (null != configForm && configForm.size() > 0) {
						...
						for (AssetConfigForm assetConfigForm : configForm) {
							...
							String mfgDate = assetConfigForm
									.getDateOfManfacturing();
							String receiptDate = assetConfigForm
									.getDateOfReciept();
							...
							assetConfigForm.setDateOfReciept(convertor
								...
								System.out.println("Update Query="+halService.update(
										emmsDataForm, assetConfigForm));
				...
				try {
					if (null != configForm && configForm.size() > 0) {
						...
						for (AssetConfigForm assetConfigForm : configForm) {
							...
							System.out
									.println(assetConfigForm.getInstalledPN());
				...
				System.out.println("FreezeRecordID"+this.emmsDataForm
							.getSelectedRecordId());
				emmsDataForm.setInstallableFormList(installAssetController
			             .fetchInstallableDetails(this.emmsDataForm
							.getSelectedRecordId(),this.emmsDataForm));
				List<InstallableAssetForm> installableFormList = emmsDataForm
				...
				try {
					if (null != installableFormList && installableFormList.size() > 0) {
						...
						for (InstallableAssetForm installableForm : installableFormList) {
							...
							System.out
									.println(installableForm.getInstallablePN());
				...
				String filename = "C:\\Users\\Public\\Desktop\\AssetConfiguration.xlsx";
				...
				if (headerStatus && countRows) {
					...
					for (int j = 1, i = 0; j <= worksheet.getLastRowNum(); j++, i++) {
						...
						configForm.setIndicator(oldConfigList.get(i)
								.getIndicator());
						configForm.setLcn(oldConfigList.get(i).getLcn());
						configForm.setPosition(oldConfigList.get(i)
								.getPosition());
						configForm.setBuildItem(oldConfigList.get(i)
								.getBuildItem());
						configForm.setAssetNum(oldConfigList.get(i)
								.getAssetNum());
						configForm
								.setPartNum(oldConfigList.get(i).getPartNum());
						configForm.setPartDescription(oldConfigList.get(i)
								.getPartDescription());
						configForm.setSerialNum(oldConfigList.get(i)
								.getSerialNum());
						configForm.setInstalledPNList(assetFormList.get(i)
								.getInstalledPNList());
						configForm.setConditionCodes(assetFormList.get(i)
								.getConditionCodes());
						configForm.setRecordRowId(assetFormList.get(i)
								.getRecordRowId());
